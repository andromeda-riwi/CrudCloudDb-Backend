...
/// Necesario para monitoreo y despliegue (Docker, Kubernetes, etc)
/// </summary>
[ApiController]
...
public class HealthController : ControllerBase
{
    ...
    /// Retorna estado OK si la API está funcionando
    /// </summary>
    /// <remarks>
    /// GET /api/health
    /// </remarks>
    ...
    /// <summary>
    /// Endpoint de health check detallado (requiere autenticación)
    ...
    {
        _logger.LogDebug("Detailed health check solicitado");
        return Ok(new
        {
            status = "healthy",
            timestamp = DateTime.UtcNow,
            version = "1.0.0",
            environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") ?? "Development",
            uptime = "calculado dinámicamente"
        });
    }
}
